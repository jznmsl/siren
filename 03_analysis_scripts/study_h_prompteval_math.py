"""
PromptEval vs M7 — Faithful Point-Estimate Comparison (MMLU-Pro Math)
======================================================================
Same Algorithm 1 (Rasch IRT + pIRT) and Algorithm 2 (two-way balanced
sampling) as study_h_prompteval_law.py.  No CI is invented for PromptEval.
...
NOTE on overlap: Original RS has Gemini-2.5-flash but no Qwen3-8B; DSPy RS
has Qwen3-8B but no Gemini.  This script restricts to the 11 overlap models
present in BOTH datasets, dropping Gemini, matching the Math tables in the
paper.

Required input layout (relative to working directory):
  results/multimodel/<MODEL>/                          # custom random search (no _law/_math suffix = Math)
    tensor_a_B500000.npy ... tensor_a_B6500000.npy
    study_e_results.json
  results/dspy_randsearch_multimodel/<MODEL>_math/     # DSPy
    tensor_a_B500000.npy ... tensor_a_B6500000.npy
    study_e_dspy_randsearch_results.json

Usage:
    cd ~/auto_research/llm_evaluation
    python study_h_prompteval_math.py

Outputs:
    results/study_h_prompteval_math_results.json     (per-cell results)
...
# 11 overlap models present in BOTH original RS Math AND DSPy RS Math
# Original RS Math dirs: NO _math suffix (early run, before suffix convention)
# DSPy RS Math dirs: _math suffix
# Original RS dropped: gemini-2.5-flash (only in original, not in DSPy)
# DSPy RS dropped: nothing (Qwen3-8B included via different orig file)

NAME_MAP_ORIG = {
    # Original Math results live in results/multimodel/<MODEL>/  (NO suffix)
    '01-ai_Yi-1.5-6B-Chat': 'Yi-1.5-6B',
    ...
    'THUDM_glm-4-9b-chat': 'GLM-4-9B',
    'internlm_internlm2_5-7b-chat': 'InternLM2.5',
    'meta-llama_Llama-3.1-8B-Instruct': 'Llama-3.1',
    'microsoft_Phi-3.5-mini-instruct': 'Phi-3.5',
    'mistralai_Mistral-7B-Instruct-v0.1': 'Mistral-v0.1',
    'mistralai_Mistral-7B-Instruct-v0.3': 'Mistral-v0.3',
}
NAME_MAP_DSPY = {
    # DSPy Math results live in results/dspy_randsearch_multimodel/<MODEL>_math/
    '01-ai_Yi-1.5-6B-Chat_math': 'Yi-1.5-6B',
    'Qwen_Qwen2-7B-Instruct_math': 'Qwen2-7B',
    'Qwen_Qwen2.5-3B-Instruct_math': 'Qwen2.5-3B',
    'Qwen_Qwen2.5-7B-Instruct_math': 'Qwen2.5-7B',
    'Qwen_Qwen3-8B_math': 'Qwen3-8B',
    'THUDM_glm-4-9b-chat_math': 'GLM-4-9B',
    'internlm_internlm2_5-7b-chat_math': 'InternLM2.5',
    'meta-llama_Llama-3.1-8B-Instruct_math': 'Llama-3.1',
    'microsoft_Phi-3.5-mini-instruct_math': 'Phi-3.5',
    'mistralai_Mistral-7B-Instruct-v0.1_math': 'Mistral-v0.1',
    'mistralai_Mistral-7B-Instruct-v0.3_math': 'Mistral-v0.3',
}

TUNERS = {
    'random_search': {
        'dir':  'multimodel',
        'name_map': NAME_MAP_ORIG,
        'study_e_filename': 'study_e_results.json',
    },
    'dspy': {
        'dir':  'dspy_randsearch_multimodel',
        'name_map': NAME_MAP_DSPY,
...
# ============== Special-case loader: Qwen3-8B Math original ==============
# The Qwen3-8B Math original RS output dir is "results/Qwen3 8B/study_e_overnight/"
# (not in results/multimodel/<MODEL>/), and the JSON lacks the wrapper structure.
# We attach Qwen3-8B's tensors and ground truth from there if standard path missing.
QWEN3_OVERRIDE = {
    'study_e_dir': Path("results/Qwen3 8B/study_e_overnight/"),
    'gt_filename': 'ground_truth_results.json',  # has theta_true_a/b
    'study_e_filename': 'study_e_overnight_results.json',
}
...
for tuner_name, tuner_cfg in TUNERS.items():
    ...
    print(f"\n{'='*90}\nTuner: {tuner_name}\n{'='*90}")
    name_map = tuner_cfg['name_map']
    for model_full, model_short in name_map.items():
        d = RESULTS_BASE / tuner_cfg['dir'] / model_full
        se_path = d / tuner_cfg['study_e_filename']
        # Special case: Qwen3-8B in random_search is in a non-standard dir
        use_special_qwen3 = (
            tuner_name == 'random_search'
            and model_short == 'Qwen3-8B'
            and not se_path.exists()
        )
        if use_special_qwen3:
            d = QWEN3_OVERRIDE['study_e_dir']
            se_path = d / QWEN3_OVERRIDE['study_e_filename']
            gt_path = d / QWEN3_OVERRIDE['gt_filename']
            if not se_path.exists() or not gt_path.exists():
                print(f"  [skip] {model_short}: missing Qwen3 special files")
                continue
            with open(se_path) as f:
                study_raw = json.load(f)
            with open(gt_path) as f:
                gt_data = json.load(f)
            # Reshape: study_raw is dict[B] -> {m7, comparison, top_k_scores}
            # gt_data  is dict[B] -> {theta_true_a, theta_true_b, ...}
            results_dict = {}
            for B_str in study_raw:
                if B_str in gt_data:
                    results_dict[B_str] = dict(study_raw[B_str])
                    results_dict[B_str]['ground_truth'] = gt_data[B_str]
            study = {'results': results_dict}
        else:
            if not se_path.exists():
                print(f"  [skip] {model_short}: missing {se_path.name} ({d})")
                continue
            with open(se_path) as f:
                study = json.load(f)
...
# Save
out_path = RESULTS_BASE / 'study_h_prompteval_math_results.json'
...
# ====================== Aggregate ======================
print("\n" + "=" * 90)
print("AGGREGATE — PromptEval vs M7 on MMLU-Pro Math (88 cells per method)")
print("=" * 90)
